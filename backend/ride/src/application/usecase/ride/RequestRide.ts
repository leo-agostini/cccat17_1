...
export default class RequestRide implements UseCase {
  ...
  async execute(input: Input): Promise<Output> {
    const account = await this.accountGateway.getAccountById(
      input.passengerId
    );

    if (!account.isPassenger) {
      throw new Error("This account is not from a passenger");
    }
    ...
    if (hasActiveRide) {
      throw new Error("This passenger has an active ride");
    }
